...
import { POOL_EVENTS_QUERY } from "@/app/lib/graphql/matroid";
...
type PoolEventsResponse = { poolEvents: RawPoolEvent[] };
...
const fetchEvents = async (bucket: number): Promise<RawPoolEvent[]> => {
  const data = await fetchMatroidGraphQL<PoolEventsResponse>({
