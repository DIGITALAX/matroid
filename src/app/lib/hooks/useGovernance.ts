...
export const useGovernance = () => {
  ...
  const base = { address: governance as `0x${string}`, abi: govAbi };
  ...
  const approveMona = async (amount: bigint) => {
    if (!isAddr(addresses.Mona) || amount === 0n) return;
  ...
  const propose = async (
    ...
    if (!guard()) return;
    if (minProposeStake > 0n) await approveMona(minProposeStake);
    return writeContractAsync({
      ...base,
  ...
  const cancelVote = async (id: bigint) => {
    if (!guard()) return;
    return writeContractAsync({ ...base, functionName: "cancelVote", args: [id] });
  };

  const execute = async (id: bigint) => {
    if (!guard()) return;
    return writeContractAsync({ ...base, functionName: "execute", args: [id] });
  };

  const withdraw = async (id: bigint) => {
    if (!guard()) return;
    return writeContractAsync({ ...base, functionName: "withdraw", args: [id] });
  };
